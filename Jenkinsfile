    ...
    stage('Containerize') {
      steps {
        container('kaniko') {
          ...
          sh "/kaniko/executor --dockerfile Dockerfile --context `pwd` --skip-tls-verify --force --destination=${env.HARBOR_URL}/devsecops/samples/spring-petclinic:v1.0.${env.BUILD_ID}"
        }
      }
    }
